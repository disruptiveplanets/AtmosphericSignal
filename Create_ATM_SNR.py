...
for i in range(len(Name)):
    ...
    if np.isfinite(PlanetaryMass[i]) and not(np.isfinite(PlanetaryRad[i])):
        PlanetaryRad[i], _, _ = mr.Mstat2R(PlanetaryMass[i], 1e-9, sample_size=10, unit='Earth')
        ...
        PlanetaryMass[i], _, _ = mr.Rstat2M(PlanetaryRad[i], 1e-9, sample_size=10, grid_size=1000,unit='Earth')  
...
SelectIndex1 = np.where(Name=="K2-315 b")


SNR = np.log10(S/S[SelectIndex])
...
plt.scatter(T_Eff_Planet[0], S[0], s=(PlanetaryRad[0]*10000)**0.5, edgecolor='navy', lw=2, zorder=20)
plt.scatter(T_Eff_Planet[1], S[1], s=(PlanetaryRad[1]*10000)**0.5, edgecolor='navy', lw=2, zorder=20)

plt.text(T_Eff_Planet[0]+30, S[0], "TOI-4306 b", va="center", color="navy" )
plt.text(T_Eff_Planet[1]+30, S[1], "SPECULOOS-2 b", va="center", color="navy")



plt.scatter(T_Eff_Planet[SelectIndex], S[SelectIndex], s=(PlanetaryRad[SelectIndex]*10000)**0.5, edgecolor='red', lw=2, zorder=20)
plt.text(T_Eff_Planet[SelectIndex]+30, S[SelectIndex], "TRAPPIST-1 b", va="center", color="red")


Scatter = plt.scatter(T_Eff_Planet, S, c=SNR, s=(PlanetaryRad*10000)**0.5, edgecolor='None', cmap='brg')
cbar = plt.colorbar()
